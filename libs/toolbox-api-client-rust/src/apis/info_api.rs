...
/// struct for typed errors of method [`get_user_home_dir`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUserHomeDirError {
...
/// struct for typed errors of method [`get_work_dir`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetWorkDirError {
    UnknownValue(serde_json::Value),
}


/// Get the current user home directory path.
pub async fn get_user_home_dir(configuration: &configuration::Configuration, ) -> Result<models::UserHomeDirResponse, Error<GetUserHomeDirError>> {

    let uri_str = format!("{}/user-home-dir", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetUserHomeDirError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Get the current daemon version
pub async fn get_version(configuration: &configuration::Configuration, ) -> Result<std::collections::HashMap<String, String>, Error<GetVersionError>> {
...
pub async fn get_work_dir(configuration: &configuration::Configuration, ) -> Result<models::WorkDirResponse, Error<GetWorkDirError>> {

    let uri_str = format!("{}/work-dir", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetWorkDirError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
